...
Public Sub Setup(S1 As Long, S2 As Long)
'set the width and height
W = Form1.src1.ScaleWidth
H = Form1.src1.ScaleHeight
...
BBO = CreateCompatibleBitmap(GetDC(0), Form1.ScaleWidth, Form1.ScaleHeight)
...
BitBlt BB, 0, 0, Form1.ScaleWidth, Form1.ScaleHeight, Form1.hdc, 0, 0, vbSrcCopy
...
Public Sub BLTIT(ByVal X As Long, ByVal Y As Long)
...
If X >= Form1.ScaleWidth - (W + 1) Then
    X = Form1.ScaleWidth - (W + 1)
End If
If Y >= Form1.ScaleHeight - (H + 1) Then
    Y = Form1.ScaleHeight - (H + 1)
End If
If Y <= 0 Then
    Y = 0
End If
...
BitBlt Form1.hdc, LX, LY, W, H, BB, LX, LY, vbSrcCopy

'blt the alpha values to the screen
SetDIBitsToDevice Form1.hdc, X, Y, W, H, 0, 0, 0, H, tmpBits(1), nSRCI, DIB_RGB_COLORS
